...
- [x] **관련 레시피 추천** (태그 + 재료 유사도) — PR (this) — `app/services/recommendation.py` 신규: 카테고리 정확일치 (`keyword`/`menu_type`/`region`/`diet`/`era`) + 재료 Jaccard 유사도 + `source_document_id` 동일 보너스를 가중합해 점수화. 시드와 동일하거나 빈 필드 양쪽이 매칭되는 가짜-양성은 명시적으로 0점. `GET /v1/private/recipes/{id}/related?limit=` (1≤limit≤20, 기본 5) 가 점수 내림차순 + `created_at` desc tie-break 으로 정렬해 `RelatedRecipeOut` 리스트 반환. **노출 규칙**: 본인 레시피는 상태 무관 노출, 타인 레시피는 `status=approved` 만 노출 — pending/rejected 누출 방지. 빈 결과(점수 0인 후보만 있을 때)는 `[]` 반환. 프론트: `RecipeDetail` 페이지 하단에 "관련 레시피" 섹션 추가 (`RelatedRecipesSection` — 2-column 카드 그리드, 유사도 점수 + 상태 배지 + 지역/메뉴 태그). 새 API 헬퍼 `api.getRelatedRecipes(id, limit)`. 17 신규 unit/integration 테스트 (스코어 weight 순서, 가시성 isolation, limit clamp, 404/401 envelope). 차세대 옵션: 기존 Gemini embeddings + pgvector 인프라 (PR #42/#44) 위에 벡터 유사도 변형을 동일 함수 시그니처로 추가 가능 (재료 코퍼스가 한국어 한정이라 카테고리 매칭만으로도 우선 만족스러운 신호).
- [x] **관련 레시피 추천 — 벡터 유사도 변형** — PR (this) — `app/services/recipe_embeddings.py` 신규 (canonical text 포매터, `embed_recipe`/`store_recipe_embedding`/`ensure_recipe_embedding`, `cosine_similarity`) + `find_related_recipes()` 시그니처 그대로 두고 dispatch 추가 (`RECOMMENDATION_PROVIDER=vector` 기본, `tag` 도 A/B 용으로 보존). 벡터 백엔드는 PR #42/#44 의 `EmbeddingAdapter`(mock / Gemini text-embedding-004 / Vertex text-embedding-005) 를 그대로 사용 — 새 키 / 새 인프라 불필요. 저장: `recipes.embedding_values` JSON 컬럼 (Alembic `0003_recipe_embedding`, Postgres 에서는 native `vector(768)` 컬럼 + HNSW cosine 인덱스 미러). `/v1/private/recipes/generate` 훅에서 생성 직후 동기 임베딩 → 다음 요청부터 vector path 이용 가능. Legacy row 는 seed 호출 시 lazy backfill + 전체 백필 admin endpoint `POST /v1/admin/recipes/embeddings/backfill` (`?force=true` 로 임베딩 텍스트 포맷 변경 후 전체 re-embed). 가시성 / 정렬 / limit / 응답 스키마는 PR #46 과 동일. **Graceful fallback**: 벡터 path 가 0건 반환하면 (seed embedding 없음 또는 모든 후보가 임베딩 미보유) 자동으로 tag scorer 로 회귀 — 임베딩 어댑터 장애 중에도 패널이 비지 않음. 20 신규 unit/integration/backfill 테스트 + 기존 17 PR #46 테스트도 모두 통과 (총 37). 1194/1194 backend tests + ruff clean + tsc clean + build clean.
- [ ] 재료 / 단계 사용자 편집
