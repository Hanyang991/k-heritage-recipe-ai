...
- [ ] pgvector 확장 native KNN 가속 — `vector_search_datapoints` 테이블에 `embedding vector(768)` 컬럼 + IVFFlat / HNSW 인덱스 추가하는 Alembic 마이그레이션 + `PgVectorSearchAdapter.query` 의 `ORDER BY embedding <=> :v` 패스트패스. 코퍼스가 ~1M 벡터를 넘으면 진행 (현재 ~40만 미만이라 브루트포스로 충분).
