...
### Vector search — per-source namespace indexing

Semantic retrieval over the heritage corpus uses **one logical index per source** (jangseogak / koreanstudies / nlk / gihohak / nihc) — a deliberate continuation of the heritage layer's per-source isolation contract so KOGL attribution stays clean and operators can rebuild or retire one source's index without touching the others.

| Service | Env var | `mock` (default) | `gemini` / `pgvector` (**free**) | `live` |
| --- | --- | --- | --- | --- |
| Embeddings | `EMBEDDING_PROVIDER` | Deterministic hash → unit-norm vector | Google AI Studio `text-embedding-004` via `:embedContent` REST | Vertex AI `text-embedding-005` via `:predict` REST |
| Vector index | `VECTOR_SEARCH_PROVIDER` | In-memory cosine, namespace-isolated dict | Postgres-backed `vector_search_datapoints` table + brute-force cosine | Vertex AI Vector Search (`upsertDatapoints` + `findNeighbors`) |
