...
_FOOD_DENYLIST_PATTERNS: tuple[str, ...] = (
    ...
    # 지명 / 국가명 (PR #28) — bare 국가/도시 토큰이 RSS / 뉴스에서 단독으로
    # 떠올라 candidate pool 에 leak 되는 경우만 차단. 합성어 (``홍콩식
    # 디저트``, ``이탈리아 디저트``, ``베트남 커피``) 는 whitespace-strip
    # 후 ``홍콩식디저트`` / ``이탈리아디저트`` 가 되어 substring 으로
    # 빠져나가므로 ``^...$`` 앵커로 *오직 bare 국가명* 만 매칭한다.
    # ``두바이`` 는 의도적으로 제외 — ``두바이쫀득쿠키`` 가 이 코드베이스
    # 의 상징적 novelty trend 이고 bare ``두바이`` 도 식품 컨텍스트에
    # 적극적으로 살려두고 싶다.
    r"^(홍콩|대만|미국|중국|유럽|영국|독일|러시아|이탈리아|베트남|프랑스|일본|싱가포르|호주|캐나다|인도|스페인|튀르키예|폴란드|네덜란드)$",
    # 산업 카테고리 (PR #28) — bare 산업/카테고리 토큰. ``뷰티 디저트``
    # 처럼 합성된 컨텍스트는 whitespace-strip 후 substring 매칭이 안되어
    # 통과한다 (``뷰티디저트`` 는 ``^뷰티$`` 에 안 걸림).
    r"^(뷰티|패션|명품|화장품|의류|가전|건설|반도체|이차전지|배터리|중소기업|버킨백)$",
    # 예능 / TV 프로그램 (PR #28) — KBS ``편스토랑`` 같이 매주 음식
    # 컨셉으로 뉴스에 오르내리지만 *프로그램 자체* 가 트렌드는 아닌
    # 케이스. 4음절 이상 고유명이라 substring collision 위험 낮음.
    r"편스토랑|런닝맨|무한도전|놀면뭐하니|복면가왕|미운우리새끼|아는형님|유퀴즈온더블록",
)
...
_BARE_PERSON_NAME_DENYLIST: tuple[str, ...] = (
    ...
    "전현무",
    "도경완",
    # 배우 (PR #28 — 라이브 RSS 에서 bare name leak)
    "송일국",
    # 종교/학계 (PR #28 — ``박형룡`` 신학자 bare leak)
    "박형룡",
    # 축구 선수 추가 (PR #28)
    "이동국",
